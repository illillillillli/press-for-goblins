(function () {
  ...
  function eventState(event) {
    ...
    var screen = project(point);
    var opacity = Math.max(0, Math.min(1, (point.z + .18) / .34));
  ...
  function drawLeader(state, button, time) {
    ...
    button.style.setProperty('--depth-opacity', (state.event.id === selectedId ? 1 : state.opacity).toFixed(3));
  ...
  function draw(time) {
    ...
    var states = assignments.map(function (id) {
      var event = events.find(function (item) { return item.id === id; });
