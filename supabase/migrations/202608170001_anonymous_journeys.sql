...
create table if not exists dashboard.session_event (
  ...
  device_label text check (device_label is null or device_label ~ '^[a-f0-9]{6}$'),
  primary key (session_hash, metric, value)
);
create index if not exists session_event_recent_idx on dashboard.session_event (occurred_at desc);
alter table dashboard.session_event enable row level security;
alter table dashboard.session_event force row level security;
revoke all on dashboard.session_event from public, anon, authenticated;
...
drop function if exists public.dashboard_ingest(text,text,text,text);
...
begin
  ...
  if v_owner_device is not null then
    v_owner_class := 'owner';
  end if;
...
begin
  delete from dashboard.session_event where occurred_at < now() - interval '30 days';
