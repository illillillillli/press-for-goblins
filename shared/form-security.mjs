...
export async function enforceEmailLimits(kind, ip, email) {
  const ipSecret = process.env.FORM_RATE_LIMIT_SECRET || process.env.UPSTASH_REDIS_REST_TOKEN;
  if (!ipSecret) throw new Error('rate limit not configured');
  const ipHash = createHmac('sha256', ipSecret).update(String(ip)).digest('hex').slice(0, 24);
  const emailHash = createHash('sha256').update(email).digest('hex').slice(0, 24);
  const day = new Date().toISOString().slice(0, 10);
  const ipKey = `pfg:${kind}:ip:${ipHash}`;
...
export async function sendProviderEmail(apiKey, payload, idempotencyKey) {
  try {
    ...
    return { ok: false, id: null, error: 'provider unavailable' };
  }
}

export function isConfiguredTestAddress(email) {
  const configured = String(process.env.PFG_TEST_EMAILS || '')
    .split(',')
